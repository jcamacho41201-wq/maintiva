...
describe("editable vehicle service intervals", () => {
  ...
  it("updates derived opportunities from effective intervals and completion", () => {
    const dueRecord = record();
    const opportunities = buildRevenueOpportunities(state([dueRecord]));
    ...
    expect(buildRevenueOpportunities(state([completedRecord]))).toHaveLength(0);
  });
});
