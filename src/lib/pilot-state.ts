...
const baselineOutreachRecordSelect = {
  ...
  smsDeliveredAt: true,
  smsFailureCode: true,
  smsFailureMessage: true,
  smsIdempotencyKey: true,
  createdAt: true,
...
export async function buildPilotState(context: AuthenticatedShopContext): Promise<DemoState> {
  ...
  return {
    ...
    customers: shop.customers.map((customer): Customer => ({
      ...
      smsConsentRecordedAt: iso(customer.smsConsentRecordedAt) || undefined,
      smsConsentRecordedByUserId: customer.smsConsentRecordedByUserId ?? undefined,
    ...
    outreachRecords: shop.outreachRecords.map((record): OutreachRecord => ({
      ...
      appointmentRequestLinkId: record.appointmentRequestLinkId ?? undefined,
      performedByUserId: record.performedByUserId ?? undefined,
      smsRecipientPhone: record.smsRecipientPhone ?? undefined,
      smsProvider: record.smsProvider ?? undefined,
      smsDeliveryStatus: record.smsDeliveryStatus ?? undefined,
      smsSentAt: iso(record.smsSentAt) || undefined,
      smsDeliveredAt: iso(record.smsDeliveredAt) || undefined,
      smsFailureCode: record.smsFailureCode ?? undefined,
      smsFailureMessage: record.smsFailureMessage ?? undefined,
      smsIdempotencyKey: record.smsIdempotencyKey ?? undefined,
      providerExternalId: record.providerExternalId ?? undefined,
      status: record.status,
    })),
